...
def setup_logger() -> logging.Logger:
    ...
    log_file  = LOG_DIR / f"copter_UNIFIED_{stamp}.log"
...
class GroundStationApp(ctk.CTk):
    def __init__(self) -> None:
        super().__init__()
        self.title("Cobra Copter — Diagnostic Ground Station  [Windows]")
        self.geometry("1280x800")
        self.minsize(960, 600)

        log.info("═══════════════════════════════════════════════════")
        log.info("   DIAGNOSTIC BOOT INITIATED (ArduCopter Edition)")
        log.info("═══════════════════════════════════════════════════")
    ...
    def _build_ui(self) -> None:
        ...
        self.lock_btn = ctk.CTkButton(
            ctrl, text="TARGET: LOITER", fg_color="#c0392b", hover_color="#922b21",
            font=("Consolas", 14, "bold"), command=self._cmd_loiter
        )
    ...
    # ─── Flight Mode Methods (ArduCopter Mappings) ───────────────────────────

    def _cmd_auto(self) -> None:
        # ArduCopter AUTO Mode = 3
        self._set_flight_mode("AUTO", 3)

    def _cmd_guided(self) -> None:
        # ArduCopter GUIDED Mode = 4
        self._set_flight_mode("GUIDED", 4)

    def _cmd_loiter(self) -> None:
        # ArduCopter LOITER Mode = 5
        if not self.active_target_gps:
            self._log_ui("⚠️ ABORT: No target acquired in FOV.")
            return
        self._set_flight_mode("LOITER", 5)
